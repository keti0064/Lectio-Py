...
# klasse til at håndtere bruger data
class Client:
...
class Besked:
    ...
    def getMessageDialog(self, Client: Client):
        ...
        for message in messages:
            ...
            titel = messageContentDiv.find("div", attrs={"class": "message-replysum-header-menu"}).findAll("div")[
                0].text.strip("\n\n")
            titel = re.sub("[\t\r\n]", "", titel)
    ...
    def replyToMessage(self, Client: Client, titel, besked):
        ...
        send_message(Client, [], titel, besked,
                     (self.getMessageViewStatexKey(Client), len(self.getMessageDialog(Client)) + 2))

    # metode bruges både til at sende nye beskeder og til replies


# send besked, hvis du sender en ny besked skal viewstatexkey bare være tom
def send_message(Client: Client, modtager, titel, besked, viewstatexkey=""):
    ...
    if viewstatexkey == "":
        postUrl = "https://www.lectio.dk/lectio/{0}/beskeder2.aspx?mappeid=-70".format(schoolID)
        soup = getSoup(postUrl, session)
        # først skal viewstatex value findes:
        viewstatexkey = soup.find("input", id="__VIEWSTATEY_KEY")["value"]
        print("effdhbdvfjikv", viewstatexkey)
        ...
        print("viewStatex: ", viewstatexkey)
    # data til reply
    else:
        ...
        data = {
            'time': '0',
            '__EVENTTARGET': 's$m$Content$Content$MessageThreadCtrl$MessagesGV$ctl{0}$SendMessageBtn'.format(
                messageNum),
            '__EVENTARGUMENT': '',
            '__LASTFOCUS': '',
            '__SCROLLPOSITION': '{"tableId":"","rowIndex":-1,"rowScreenOffsetTop":-1,"rowScreenOffsetLeft":-1,"pixelScrollTop":0,"pixelScrollLeft":0}',
            ...
            's$m$searchinputfield': '',
            's$m$Content$Content$ListGridSelectionTree$folders': '-70',
            's$m$Content$Content$MessageThreadCtrl$MessagesGV$ctl{0}$EditModeHeaderTitleTB$tb'.format(
                messageNum): titel,
            's$m$Content$Content$MessageThreadCtrl$MessagesGV$ctl{0}$EditModeContentBBTB$TbxNAME$tb'.format(
                messageNum): besked,
            's$m$Content$Content$MessageThreadCtrl$MessagesGV$ctl{0}$AttachmentDocChooser$selectedDocumentId'.format(
                messageNum): '',
            'masterfootervalue': 'X1!ÆØÅ',
            'LectioPostbackId': '',
        }
    ...
    for m in modtager:
        ...
        print("new statex: " + viewstatexkey)
    ...
    for dagNum, dag in enumerate(ugeModuler):
        ...
        for a_num, a_tag in enumerate(dag):
            ...
            if data != None:
                ...
                if Lokale_Index != -1:
                    modul_lokale = a_tag_additional_info[
                                   Lokale_Index:a_tag_additional_info.find("\n", Lokale_Index + 8)]
                else:
                    modul_lokale = ""
                ...
                # da eksamensmoduler fungere helt anderledes skal der andet logik til
                if modul_status == "s2bgboxeksamen":
                    try:
                        modul_titel = data.find("div", attrs={"class": "s2skemabrikcontent s2normal"}).find("span",
                                                                                                            style="word-wrap:break-word;").text
                    except:
                        modul_titel = "fejl :("

                    try:
                        modul_dato_tid = data.find("div", attrs={"class": "s2timeline"}).text
                    except:
                        modul_titel = "fejl :("

                # lav modul objekt og tilføj til liste
                denneDag.append(
                    Modul(modul_lære, modul_hold, modul_pos, modul_lokale, modul_dato_tid, modul_status, modul_ID,
                          modul_titel))
                # slut modul
            else:
                continue
...
class Modul:
    ...
    def get_site_data(self, Client: Client):
        skoleID = Client.schoolID
        elevID = Client.elevID
        if self.status == "s2bgboxeksamen":
            return "dumme lorte eksamen"
        # Her behøves der ikke at anskaffes viewstatex, get requesten kan bare køres direkte
        siteURL = "https://www.lectio.dk/lectio/{0}/aktivitet/aktivitetforside2.aspx?absid={1}&prevurl=SkemaNy.aspx&elevid={2}".format(
            skoleID, self.id, elevID)

        soup = getSoup(siteURL, Client.session)
        containerDiv = soup.find("div", id="homeworkContentContainer")
        titelDiv = containerDiv.find("div", id="s_m_Content_Content_tocAndToolbar_actHeader")
        contentDiv = containerDiv.find("div", id="s_m_Content_Content_tocAndToolbar_inlineHomeworkDiv")
        textArea = containerDiv.find("textarea")
        try:
...
def get_all_opgaver(Client: Client):
    ...
    for row in allRows[1::]:
        ...
        allOpgaver.append(
            Opgave(uge, hold, opgavetitel, link, frist, elevtid, status, fravær, afventer, opgavenote, karakter,
                   elevnote))
...
class Opgave():
    ...
    def __init__(self, uge, hold, opgavetitel, link, frist, elevtid, status, fravær, afventer, opgavenote, karakter,
                 elevnote):
        self.uge = uge,
        self.hold = hold
